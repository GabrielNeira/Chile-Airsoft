-- Hotfix: permitir acceso a Operaciones Cancha para rol field_admin en user_roles.
...
-- Diagnostico (SQL editor, reemplaza correo):
-- select u.id, lower(u.email) as email
-- from auth.users u
-- where lower(u.email) = 'usuario@correo.cl';
--
-- select ur.user_id, ur.role, ur.user_role, ur.created_at
-- from public.user_roles ur
-- where ur.user_id = '<USER_ID_AQUI>'::uuid
-- order by ur.created_at desc;
--
-- select fa.field_id, fa.user_id, fa.assigned_at
-- from public.field_admins fa
-- where fa.user_id = '<USER_ID_AQUI>'::uuid;
