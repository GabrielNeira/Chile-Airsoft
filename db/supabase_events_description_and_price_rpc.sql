-- 1. Añadir columna description a public.events
ALTER TABLE public.events ADD COLUMN IF NOT EXISTS description text;

-- 2. Actualizar la función RPC para incluir address, google_maps_url, price, starts_at y description
...
begin
  ...
  select exists (
    ...
    where c.table_schema = 'public'
      and c.table_name = 'events'
      and c.column_name = 'starts_at'
  ) into v_has_starts_at;
  ...
  v_price_expr := case when v_has_price then 'e.price' else '25000::integer' end;
  v_starts_at_expr := case when v_has_starts_at then 'e.starts_at' else 'null::timestamptz' end;
  v_description_expr := case when v_has_description then 'e.description' else 'null::text' end;
  ...
  v_sql := '
    with base_events as (
      select
        ...
        (' || v_price_expr || ')::integer as price,
        (' || v_starts_at_expr || ')::timestamptz as starts_at,
        (' || v_description_expr || ')::text as description
